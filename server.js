...
var mp = new moneypot.Session(config.moneypot);
function onMpConnect(error) {
  if (error) {
    logger.error('Unable to connect to MoneyPot: ' + error.message);
    process.exit(1);
  }
...
if (config.moneypot.cookie) {
  mp.connect(onMpConnect);
} else {
  mp.getCookie(function(error, cookie) {
    if (error) {
      logger.error('Unable to acquire new session cookie: ' + error.message);
      process.exit(1);
    }
    logger.info('Acquired new session cookie.');
    console.log(cookie);
    mp.connect(onMpConnect);
  });
}
...
mp.socket.on('game_crash', function(data) {
  var id = mp.game.id;
  models.Game.find({
...
mp.socket.on('msg', function(message) {
  switch (message.type) {
    case 'say':
      break;
    case 'mute':
